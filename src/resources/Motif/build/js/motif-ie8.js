    ...
    Gauntlet.prototype = {
        ...
        inputError: function(input) {
            if (input.type === "checkbox" || input.type === "radio") {
                ...
                input.$elem.closest("label").removeClass(this.options.labelClasses.success).addClass(this.options.labelClasses.error);
        ...
        addMessage: function(input, addAfter) {
            var elem = addAfter || input.$elem, currentMessage = elem.next("." + this.options.alertClasses.error);
    ...
    AjaxSubmission.prototype = {
        ...
        resetForm: function() {
            if (this.options.clearForm) {
                ...
                this.$elem.find("label").removeClass(this.options.successClass);
            }
...
(function($, window, document, Motif, undefined) {
    ...
    Reveal.prototype = {
        ...
        showTargets: function() {
            var self = this, targets = typeof self.reference.targets === "function" ? self.reference.targets.call(self) : self.reference.targets;
        ...
        updateReference: function(config) {
            var self = this, updates = config || {}, reference = window.Reveal, referenceGroup = self.group ? reference.groups[self.group] : false, referenceTrigger = reference.triggers[self.identity];
...
(function($, window, document, Motif, undefined) {
    ...
    Tabs.prototype = {
        ...
        prepTransition: function(instance) {
            var currentTab = instance.reference.targets[0], tabsStyle = this.$tabs.attr("style");
            if (!tabsStyle) {
...
(function(app, win) {
    ...
    function text_size(width, height, template) {
        ...
        var scale = 1 / 12;
        var newHeight = Math.min(smallSide * .75, .75 * bigSide * scale);
        return {
    ...
    function draw_canvas(args) {
        var ctx = args.ctx, dimensions = args.dimensions, template = args.template, ratio = args.ratio, holder = args.holder, literal = holder.textmode == "literal", exact = holder.textmode == "exact";
        ...
        if (text_width / width >= .75) {
            text_height = Math.floor(text_height * .75 * (width / text_width));
        }
    ...
    function draw_svg(args) {
        var dimensions = args.dimensions, template = args.template, holder = args.holder, literal = holder.textmode == "literal", exact = holder.textmode == "exact";
        var ts = text_size(dimensions.width, dimensions.height, template);
        var text_height = ts.height;
    ...
    if (!Object.prototype.hasOwnProperty) /*jshint -W001, -W103 */
...
//# sourceMappingURL=motif-ie8.js.map